...
```bash
git clone https://github.com/<your-username>/ow-blood-pulse.git
cd ow-blood-pulse
...
6. **Settings → Video → Output Resolution** → `256 × 64`

> ⚠️ 分辨率必须是 **256×64**，这是 OCR 优化的关键参数。
...
```
  ...
  ⚔️ 掉血! HP=312/350 (89%) → 强度=11, pinch
...
```yaml
max_strength: 30    # ⚠️ 绝对硬上限，代码不可超越
...
low_hp_wave: "heartbeat"    # 低血量波形 (HP < max × 30%)
...
| 波形名 | 类型 | 时长 | 感受描述 |
|--------|------|------|----------|
| `pinch` | ⚔️ 掉血 | 700ms | 快速按捏，逐次减弱 |
| `sharp` | ⚔️ 掉血 | 300ms | 单次锐击脉冲 |
| `rapid` | ⚔️ 掉血 | 300ms | 快速三连击 |
...
- 每行 = 100ms
- `[频率×4]`：频率范围 10~240
- `[强度×4]`：强度范围 0~100（相对于 `set_strength` 的百分比）
...
A: HPParser 内置了锁定 max 和跳变过滤机制。连续几帧会自动稳定。确保 OBS 输出分辨率是 256×64。
...
256×64 单帧 OCR (limit_side_len=64, cls=OFF):

| 配置 | 延迟 | 60 FPS |
|------|------|:------:|
| OpenVINO FP32 CPU | **~2ms** | ✅ |
| RapidOCR (ONNX Runtime) | ~560ms | ❌ |
